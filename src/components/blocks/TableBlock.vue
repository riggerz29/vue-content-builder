<template>
    <div
        class="block-wrapper"
        :class="{ 'block-wrapper--selected': isSelected }"
        @click.stop="$emit('select')"
        @drop.stop="handleDrop"
        @dragover.prevent
    >
        <BlockActions
            :is-selected="isSelected"
            @select="$emit('select', block.id)"
            @update="$emit('update', block.id)"
            @delete="$emit('delete', block.id)"
            @copy="$emit('copy', block.id)"
            @move-up="$emit('move-up', block.id)"
            @move-down="$emit('move-down', block.id)"
        />

        <div class="table-block" :style="containerStyle">
            <table :style="tableStyle" class="table-block__table">
                <tbody>
                <tr v-for="(row, rowIndex) in block.properties.rows" :key="rowIndex">
                    <td
                        v-for="(cell, cellIndex) in row.cells"
                        :key="cellIndex"
                        :style="cellStyle"
                    >
                        {{ cell }}
                    </td>
                </tr>
                </tbody>
            </table>
        </div>
    </div>
</template>

<script>
import { computed } from 'vue'
import BlockActions from "../common/BlockActions.vue";

export default {
    name: 'TableBlock',
    components: {BlockActions},
    props: {
        block: { type: Object, required: true },
        isSelected: { type: Boolean, default: false },
        index: { type: Number, required: true }
    },
    emits: ['select', 'update', 'delete', 'copy', 'move-up', 'move-down', 'drop'],
    setup(props, { emit }) {
        const containerStyle = computed(() => ({
            margin: `${props.block.properties.margin.top}px ${props.block.properties.margin.right}px ${props.block.properties.margin.bottom}px ${props.block.properties.margin.left}px`
        }))

        const tableStyle = computed(() => ({
            width: '100%',
            borderCollapse: 'collapse',
            backgroundColor: props.block.properties.backgroundColor,
            color: props.block.properties.textColor,
            fontSize: `${props.block.properties.fontSize}px`
        }))

        const cellStyle = computed(() => ({
            border: `${props.block.properties.borderWidth}px solid ${props.block.properties.borderColor}`,
            padding: `${props.block.properties.cellPadding}px`
        }))

        const handleDrop = (event) => {
            emit('drop', event, props.index)
        }

        return {
            containerStyle,
            tableStyle,
            cellStyle,
            handleDrop
        }
    }
}
</script>

<style scoped>
.block-wrapper {
    position: relative;
    padding: 4px;
    margin: 2px 0;
    transition: all 0.2s;
}

.block-wrapper--selected {
    outline: 2px solid #2f4574;
    outline-offset: 2px;
}

.block-wrapper:hover {
    background: rgba(59, 130, 246, 0.05);
}

.table-block {
    padding: 10px;
}

.table-block__table {
    table-layout: fixed;
}
</style>
